...
class PiketAdapter(var callback : OnClickButtonItemPiketListListener) : RecyclerView.Adapter<PiketAdapter.PiketViewHolder>(){
    var list = ArrayList<ModelItem>()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): PiketViewHolder {
        return PiketViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.item_piket,parent,false))
    }
    ...
    override fun onBindViewHolder(holder: PiketViewHolder, position: Int) {
        ...
        holder.btnSelesai.setOnClickListener {
            callback.onClickButtonSelesaiItemPiket(list[position],holder.ivLoading, holder.btnSelesai)
        }
